...
function ListSampel(props) {
    ...
    const onSampChange = (kec, desa, sls) => {
        setSamp(prevState => {
          // Inisialisasi prevState jika belum ada
          const newState = { ...prevState };
      
          // Inisialisasi level kec jika belum ada
          if (!newState[kec]) {
            newState[kec] = {};
          }
      
          // Inisialisasi level desa jika belum ada
          if (!newState[kec][desa]) {
            newState[kec][desa] = {};
          }
      
          // Toggle nilai sls
        if (newState[kec][desa][sls] !== undefined) {
            console.log(newState[kec][desa][sls], sls , "atas");
            if (newState[kec][desa][sls] === true){
                newState[kec][desa][sls] = false
            }else{
                newState[kec][desa][sls] = true
            }
        } else {
            console.log(newState[kec][desa][sls], sls, "bawah");
            newState[kec][desa][sls] = true;
          }
                                                    ...
                                                    return (
                                                        <div key={subIndex} className="the-inside-row lg:grid lg:justify-items-end">
                                                            ...
                                                            {data2.filter((innerItem) => (innerItem.kode_desa === subItem.kode_desa) && (innerItem.kode_kec === subItem.kode_kec) ).map((innerItem,innerIndex) => {
                                                                ...
                                                                let truth = null;
                                                                if(samp && samp[innerItem.kode_kec] && samp[innerItem.kode_kec][innerItem.kode_desa] && samp[innerItem.kode_kec][innerItem.kode_desa][innerItem.kode_sls]){
                                                                    truth = samp[innerItem.kode_kec][innerItem.kode_desa][innerItem.kode_sls]
                                                                }

                                                                if(truth != null){
                                                                    // console.log(innerItem.kode_kec, innerItem.kode_desa, innerItem.kode_sls);
                                                                    // console.log(truth);
                                                                }
                                                                        ...
                                                                        <div  key={innerIndex} className="the-inside-row lg:grid lg:justify-items-end w-full">
                                                                            <div className={`p-1 md:p-2 lg:w-[88%] ml-20 my-1 bg-[#F5F4F4] rounded-md text-xs flex`}>
                                                                                ...
                                                                                <div className="flex items-center mx-auto">
                                                                                    ...
                                                                                    className="w-4 h-4" 
                                                                                    checked={false}
                                                                                    onChange={() => {onSampChange(innerItem.kode_kec, innerItem.kode_desa, innerItem.kode_sls)}}
                                                                                    />
                                                                                </div>
                                                                            </div>
